...
public class MerchantContainer extends Container
{
    ...
    public MerchantContainer(int p_i612_1_, PlayerInventory p_i612_2_)
    {
        this(p_i612_1_, p_i612_2_, new NPCMerchant(p_i612_2_.player));
    }

    public MerchantContainer(int p_i613_1_, PlayerInventory p_i613_2_, IMerchant p_i613_3_)
    {
        super(ContainerType.MERCHANT, p_i613_1_);
        this.merchant = p_i613_3_;
        this.merchantInventory = new MerchantInventory(p_i613_3_);
        this.addSlot(new Slot(this.merchantInventory, 0, 136, 37));
        this.addSlot(new Slot(this.merchantInventory, 1, 162, 37));
        this.addSlot(new MerchantResultSlot(p_i613_2_.player, p_i613_3_, this.merchantInventory, 2, 220, 37));

        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 9; ++j)
            {
                this.addSlot(new Slot(p_i613_2_, j + i * 9 + 9, 108 + j * 18, 84 + i * 18));
            }
        }

        for (int k = 0; k < 9; ++k)
        {
            this.addSlot(new Slot(p_i613_2_, k, 108 + k * 18, 142));
        }
    }
