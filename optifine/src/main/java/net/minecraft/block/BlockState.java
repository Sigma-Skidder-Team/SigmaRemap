...
public class BlockState extends StateHolder<Block, BlockState> implements IStateHolder<BlockState>, IForgeBlockState
{
    ...
    public BlockState(Block p_i160_1_, ImmutableMap < IProperty<?>, Comparable<? >> p_i160_2_)
    {
        super(p_i160_1_, p_i160_2_);
        this.lightLevel = p_i160_1_.getLightValue(this);
        this.transparent = p_i160_1_.isTransparent(this);
    }
    ...
    static final class Cache
    {
        ...
        private Cache(BlockState p_i171_1_)
        {
            Block block = p_i171_1_.getBlock();
            this.solid = block.isSolid(p_i171_1_);
            this.opaqueCube = block.isOpaqueCube(p_i171_1_, EmptyBlockReader.INSTANCE, BlockPos.ZERO);
            this.propagatesSkylightDown = block.propagatesSkylightDown(p_i171_1_, EmptyBlockReader.INSTANCE, BlockPos.ZERO);
            this.opacity = block.getOpacity(p_i171_1_, EmptyBlockReader.INSTANCE, BlockPos.ZERO);

            if (!p_i171_1_.isSolid())
            {
            ...
            else
            {
                ...
                VoxelShape voxelshape = block.getRenderShape(p_i171_1_, EmptyBlockReader.INSTANCE, BlockPos.ZERO);
            ...
            this.collisionShape = block.getCollisionShape(p_i171_1_, EmptyBlockReader.INSTANCE, BlockPos.ZERO, ISelectionContext.dummy());
            this.isCollisionShapeLargerThanFullBlock = Arrays.stream(Direction.Axis.values()).anyMatch((p_lambda$new$0_1_) ->
            {
                return this.collisionShape.getStart(p_lambda$new$0_1_) < 0.0D || this.collisionShape.getEnd(p_lambda$new$0_1_) > 1.0D;
            });
            this.solidSides = new boolean[6];

            for (Direction direction1 : DIRECTIONS)
            {
                this.solidSides[direction1.ordinal()] = Block.hasSolidSide(p_i171_1_, EmptyBlockReader.INSTANCE, BlockPos.ZERO, direction1);
            }

            this.opaqueCollisionShape = Block.isOpaque(p_i171_1_.getCollisionShape(EmptyBlockReader.INSTANCE, BlockPos.ZERO));
        }
    }
}
