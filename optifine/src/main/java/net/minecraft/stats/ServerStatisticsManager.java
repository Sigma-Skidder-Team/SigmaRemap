...
public class ServerStatisticsManager extends StatisticsManager
{
    ...
    public ServerStatisticsManager(MinecraftServer p_i1540_1_, File p_i1540_2_)
    {
        this.server = p_i1540_1_;
        this.statsFile = p_i1540_2_;

        if (p_i1540_2_.isFile())
        {
            try
            {
                this.parseLocal(p_i1540_1_.getDataFixer(), FileUtils.readFileToString(p_i1540_2_));
            }
            catch (IOException ioexception)
            {
                LOGGER.error("Couldn't read statistics file {}", p_i1540_2_, ioexception);
            }
            catch (JsonParseException jsonparseexception)
            {
                LOGGER.error("Couldn't parse statistics file {}", p_i1540_2_, jsonparseexception);
            }
        }
    }
