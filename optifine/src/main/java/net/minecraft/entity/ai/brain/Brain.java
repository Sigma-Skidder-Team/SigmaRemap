...
public class Brain<E extends LivingEntity> implements IDynamicSerializable
{
    ...
    public <T> Brain(Collection < MemoryModuleType<? >> p_i462_1_, Collection < SensorType <? extends Sensor <? super E >>> p_i462_2_, Dynamic<T> p_i462_3_)
    {
        p_i462_1_.forEach((p_218228_1_) ->
        {
            Optional optional = this.memories.put(p_218228_1_, Optional.empty());
        });
        p_i462_2_.forEach((p_218204_1_) ->
        {
        ...
        for (Entry<Dynamic<T>, Dynamic<T>> entry : p_i462_3_.get("memories").asMap(Function.identity(), Function.identity()).entrySet())
        {
