...
public class PerlinNoiseGenerator implements INoiseGenerator
{
    ...
    public PerlinNoiseGenerator(SharedSeedRandom p_i2980_1_, int p_i2980_2_, int p_i2980_3_)
    {
        this(p_i2980_1_, new IntRBTreeSet(IntStream.rangeClosed(-p_i2980_2_, p_i2980_3_).toArray()));
    }

    public PerlinNoiseGenerator(SharedSeedRandom p_i2981_1_, IntSortedSet p_i2981_2_)
    {
        if (p_i2981_2_.isEmpty())
        {
            throw new IllegalArgumentException("Need some octaves!");
        }
        else
        {
            int i = -p_i2981_2_.firstInt();
            int j = p_i2981_2_.lastInt();
            int k = i + j + 1;
            ...
            {
                SimplexNoiseGenerator simplexnoisegenerator = new SimplexNoiseGenerator(p_i2981_1_);
                int l = j;
                this.noiseLevels = new SimplexNoiseGenerator[k];

                if (j >= 0 && j < k && p_i2981_2_.contains(0))
                {
                ...
                {
                    if (i1 >= 0 && p_i2981_2_.contains(l - i1))
                    {
                        this.noiseLevels[i1] = new SimplexNoiseGenerator(p_i2981_1_);
                    }
                    else
                    {
                        p_i2981_1_.skip(262);
                ...
                if (j > 0)
                {
                    ...
                    {
                        if (j1 < k && p_i2981_2_.contains(l - j1))
                        {
