...
public class WorldGenRegion implements IWorld
{
    ...
    public WorldGenRegion(ServerWorld p_i2880_1_, List<IChunk> p_i2880_2_)
    {
        int i = MathHelper.floor(Math.sqrt((double)p_i2880_2_.size()));

        if (i * i != p_i2880_2_.size())
        {
            throw(IllegalStateException)Util.pauseDevMode(new IllegalStateException("Cache size is not a square."));
        }
        else
        {
            ChunkPos chunkpos = p_i2880_2_.get(p_i2880_2_.size() / 2).getPos();
            this.chunkPrimers = p_i2880_2_;
            this.mainChunkX = chunkpos.x;
            this.mainChunkZ = chunkpos.z;
            this.field_217380_e = i;
            this.world = p_i2880_1_;
            this.seed = p_i2880_1_.getSeed();
            this.chunkGenSettings = p_i2880_1_.getChunkProvider().getChunkGenerator().getSettings();
            this.seaLevel = p_i2880_1_.getSeaLevel();
            this.worldInfo = p_i2880_1_.getWorldInfo();
            this.random = p_i2880_1_.getRandom();
            this.dimension = p_i2880_1_.getDimension();
