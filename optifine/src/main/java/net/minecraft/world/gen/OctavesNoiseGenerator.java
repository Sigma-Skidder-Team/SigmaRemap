...
public class OctavesNoiseGenerator implements INoiseGenerator
{
    ...
    public OctavesNoiseGenerator(SharedSeedRandom p_i2740_1_, int p_i2740_2_, int p_i2740_3_)
    {
        this(p_i2740_1_, new IntRBTreeSet(IntStream.rangeClosed(-p_i2740_2_, p_i2740_3_).toArray()));
    }

    public OctavesNoiseGenerator(SharedSeedRandom p_i2741_1_, IntSortedSet p_i2741_2_)
    {
        if (p_i2741_2_.isEmpty())
        {
            throw new IllegalArgumentException("Need some octaves!");
        }
        else
        {
            int i = -p_i2741_2_.firstInt();
            int j = p_i2741_2_.lastInt();
            int k = i + j + 1;
            ...
            {
                ImprovedNoiseGenerator improvednoisegenerator = new ImprovedNoiseGenerator(p_i2741_1_);
                int l = j;
                this.octaves = new ImprovedNoiseGenerator[k];

                if (j >= 0 && j < k && p_i2741_2_.contains(0))
                {
                ...
                {
                    if (i1 >= 0 && p_i2741_2_.contains(l - i1))
                    {
                        this.octaves[i1] = new ImprovedNoiseGenerator(p_i2741_1_);
                    }
                    else
                    {
                        p_i2741_1_.skip(262);
                ...
                if (j > 0)
                {
                    ...
                    {
                        if (j1 < k && p_i2741_2_.contains(l - j1))
                        {
