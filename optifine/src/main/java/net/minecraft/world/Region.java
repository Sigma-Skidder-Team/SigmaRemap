...
public class Region implements IBlockReader, ICollisionReader
{
    ...
    public Region(World p_i1110_1_, BlockPos p_i1110_2_, BlockPos p_i1110_3_)
    {
        this.world = p_i1110_1_;
        this.chunkX = p_i1110_2_.getX() >> 4;
        this.chunkZ = p_i1110_2_.getZ() >> 4;
        int i = p_i1110_3_.getX() >> 4;
        int j = p_i1110_3_.getZ() >> 4;
        this.chunks = new IChunk[i - this.chunkX + 1][j - this.chunkZ + 1];
        AbstractChunkProvider abstractchunkprovider = p_i1110_1_.getChunkProvider();
        this.empty = true;
        ...
        for (int i1 = p_i1110_2_.getX() >> 4; i1 <= p_i1110_3_.getX() >> 4; ++i1)
        {
            for (int j1 = p_i1110_2_.getZ() >> 4; j1 <= p_i1110_3_.getZ() >> 4; ++j1)
            {
                IChunk ichunk = this.chunks[i1 - this.chunkX][j1 - this.chunkZ];

                if (ichunk != null && !ichunk.isEmptyBetween(p_i1110_2_.getY(), p_i1110_3_.getY()))
                {
                    this.empty = false;
                    return;
                }
