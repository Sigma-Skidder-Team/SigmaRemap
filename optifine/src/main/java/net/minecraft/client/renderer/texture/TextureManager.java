...
public class TextureManager implements ITickable, AutoCloseable, IFutureReloadListener
{
    ...
    public TextureManager(IResourceManager p_i112_1_)
    {
        this.resourceManager = p_i112_1_;
    }
    ...
    public CompletableFuture<Void> reload(IFutureReloadListener.IStage stage, IResourceManager resourceManager, IProfiler preparationsProfiler, IProfiler reloadProfiler, Executor backgroundExecutor, Executor gameExecutor)
    {
        ...
        return CompletableFuture.allOf(MainMenuScreen.loadAsync(this, backgroundExecutor), this.loadAsync(Widget.WIDGETS_LOCATION, backgroundExecutor)).<Void>thenCompose(stage::markCompleteAwaitingOthers).thenAcceptAsync((p_lambda$reload$4_3_) ->
        {
    ...
    {
        for (Entry<ResourceLocation, Texture> entry : new HashSet<>(this.mapTextureObjects.entrySet()))
        {
