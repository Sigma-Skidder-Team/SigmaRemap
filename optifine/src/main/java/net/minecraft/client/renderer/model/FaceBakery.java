...
public class FaceBakery
{
    ...
    private void storeVertexData(int[] faceData, int storeIndex, Vector3f positionIn, int shadeColor, TextureAtlasSprite sprite, BlockFaceUV faceUV)
    {
        ...
        int j = storeIndex * i;
        faceData[j] = Float.floatToRawIntBits(positionIn.getX());
        faceData[j + 1] = Float.floatToRawIntBits(positionIn.getY());
        faceData[j + 2] = Float.floatToRawIntBits(positionIn.getZ());
        faceData[j + 3] = shadeColor;
        faceData[j + 4] = Float.floatToRawIntBits(sprite.getInterpolatedU((double)faceUV.getVertexU(storeIndex)));
        faceData[j + 4 + 1] = Float.floatToRawIntBits(sprite.getInterpolatedV((double)faceUV.getVertexV(storeIndex)));
    }
