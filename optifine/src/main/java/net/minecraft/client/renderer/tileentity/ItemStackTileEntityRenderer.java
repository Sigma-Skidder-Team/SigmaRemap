...
public class ItemStackTileEntityRenderer
{
    ...
    public void renderRaw(ItemStack p_renderRaw_1_, MatrixStack p_renderRaw_2_, IRenderTypeBuffer p_renderRaw_3_, int p_renderRaw_4_, int p_renderRaw_5_)
    {
        ...
        else if (item == Items.SHIELD)
        {
            ...
            IVertexBuilder ivertexbuilder1 = material.getSprite().wrapBuffer(ItemRenderer.getBuffer(p_renderRaw_3_, this.modelShield.getRenderType(material.getAtlasLocation()), false, p_renderRaw_1_.hasEffect()));
            this.modelShield.func_228294_b_().render(p_renderRaw_2_, ivertexbuilder1, p_renderRaw_4_, p_renderRaw_5_, 1.0F, 1.0F, 1.0F, 1.0F);

            if (flag)
            {
                List<Pair<BannerPattern, DyeColor>> list = BannerTileEntity.func_230138_a_(ShieldItem.getColor(p_renderRaw_1_), BannerTileEntity.func_230139_a_(p_renderRaw_1_));
                BannerTileEntityRenderer.func_230180_a_(p_renderRaw_2_, p_renderRaw_3_, p_renderRaw_4_, p_renderRaw_5_, this.modelShield.func_228293_a_(), material, false, list);
            }
            else
            {
        ...
        else if (item == Items.TRIDENT)
        {
            p_renderRaw_2_.push();
            p_renderRaw_2_.scale(1.0F, -1.0F, -1.0F);
            IVertexBuilder ivertexbuilder = ItemRenderer.getBuffer(p_renderRaw_3_, this.trident.getRenderType(TridentModel.TEXTURE_LOCATION), false, p_renderRaw_1_.hasEffect());
