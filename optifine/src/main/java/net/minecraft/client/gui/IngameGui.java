...
public class IngameGui extends AbstractGui
{
    ...
    public IngameGui(Minecraft p_i239_1_)
    {
        this.mc = p_i239_1_;
        this.itemRenderer = p_i239_1_.getItemRenderer();
        this.overlayDebug = new DebugOverlayGui(p_i239_1_);
        this.spectatorGui = new SpectatorGui(p_i239_1_);
        this.persistantChatGUI = new NewChatGui(p_i239_1_);
        this.overlayPlayerList = new PlayerTabOverlayGui(p_i239_1_, this);
        this.overlayBoss = new BossOverlayGui(p_i239_1_);
        this.overlaySubtitle = new SubtitleOverlayGui(p_i239_1_);
        ...
        this.chatListeners.get(ChatType.CHAT).add(new NormalChatListener(p_i239_1_));
        this.chatListeners.get(ChatType.CHAT).add(ichatlistener);
        this.chatListeners.get(ChatType.SYSTEM).add(new NormalChatListener(p_i239_1_));
        this.chatListeners.get(ChatType.SYSTEM).add(ichatlistener);
        this.chatListeners.get(ChatType.GAME_INFO).add(new OverlayChatListener(p_i239_1_));
    ...
    private void renderAttackIndicator()
    {
        ...
        if (gamesettings.thirdPersonView == 0 && (this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR || this.isTargetNamedMenuProvider(this.mc.objectMouseOver)))
        {
    ...
    protected void renderPotionEffects()
    {
        ...
        if (!collection.isEmpty())
        {
            ...
            Iterator iterator = Ordering.<EffectInstance>natural().<EffectInstance>reverse().sortedCopy(collection).iterator();
            ...
            while (true)
            {
                ...
                if (effectinstance.isShowIcon())
                {
                    ...
                    {
                        Reflector.call(effectinstance, Reflector.IForgeEffectInstance_renderHUDEffect, this, k, l, this.getBlitOffset(), f);
                    }
                }
            }
        }
    ...
    public void renderSelectedItem()
    {
        ...
        if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty())
        {
            ...
            {
                s = Reflector.callString(this.highlightingItemStack, Reflector.IForgeItemStack_getHighlightTip, s);
            }
            ...
            if (k > 0)
            {
                ...
                {
                    i = (this.scaledWidth - fontrenderer.getStringWidth(s)) / 2;
    ...
    public void tick()
    {
        ...
        if (this.mc.player != null)
        {
            ...
            if (Reflector.IForgeItemStack_getHighlightTip.exists())
            {
                String s = Reflector.callString(itemstack, Reflector.IForgeItemStack_getHighlightTip, itemstack.getDisplayName().getUnformattedComponentText());
                String s1 = Reflector.callString(this.highlightingItemStack, Reflector.IForgeItemStack_getHighlightTip, this.highlightingItemStack.getDisplayName().getUnformattedComponentText());
                flag = Config.equals(s, s1);
            }
