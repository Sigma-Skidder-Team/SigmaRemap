    ...
    private static void saveScreenshotRaw(File gameDirectory, @Nullable String screenshotName, int width, int height, Framebuffer buffer, Consumer<ITextComponent> messageConsumer)
    {
        ...
        SimpleResource.RESOURCE_IO_EXECUTOR.execute(() ->
        {
            try {
                ...
                ITextComponent itextcomponent1 = (new StringTextComponent(file3.getName())).applyTextStyle(TextFormatting.UNDERLINE).applyTextStyle((p_lambda$null$1_1_) -> {
                    p_lambda$null$1_1_.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE, file3.getAbsolutePath()));
                });
