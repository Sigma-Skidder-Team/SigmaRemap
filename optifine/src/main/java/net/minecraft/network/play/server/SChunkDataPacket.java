...
public class SChunkDataPacket implements IPacket<IClientPlayNetHandler>
{
    ...
    public SChunkDataPacket(Chunk p_i99_1_, int p_i99_2_)
    {
        ChunkPos chunkpos = p_i99_1_.getPos();
        this.chunkX = chunkpos.x;
        this.chunkZ = chunkpos.z;
        this.fullChunk = p_i99_2_ == 65535;
        this.heightmapTags = new CompoundNBT();

        for (Entry<Heightmap.Type, Heightmap> entry : p_i99_1_.getHeightmaps())
        {
        ...
        if (this.fullChunk)
        {
            this.biomes = p_i99_1_.getBiomes().clone();
        }

        this.buffer = new byte[this.calculateChunkSize(p_i99_1_, p_i99_2_)];
        this.availableSections = this.extractChunkData(new PacketBuffer(this.getWriteBuffer()), p_i99_1_, p_i99_2_);
        this.tileEntityTags = Lists.newArrayList();

        for (Entry<BlockPos, TileEntity> entry1 : p_i99_1_.getTileEntityMap().entrySet())
        {
            BlockPos blockpos = entry1.getKey();
            TileEntity tileentity = entry1.getValue();
            int i = blockpos.getY() >> 4;

            if (this.isFullChunk() || (p_i99_2_ & 1 << i) != 0)
            {
        ...
        ChunkDataOF chunkdataof = ChunkOF.makeChunkDataOF(p_i99_1_);
