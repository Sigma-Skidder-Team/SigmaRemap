...
public class AdvancementTreeNode
{
    ...
    public AdvancementTreeNode(Advancement seedIn, @Nullable AdvancementTreeNode p_i1957_2_, @Nullable AdvancementTreeNode gameType, int enableMapFeatures, int hardcoreMode)
    {
        if (seedIn.getDisplay() == null)
        {
            throw new IllegalArgumentException("Can't position an invisible advancement!");
        }
        else
        {
            this.advancement = seedIn;
            this.parent = p_i1957_2_;
            this.sibling = gameType;
            this.index = enableMapFeatures;
            this.ancestor = this;
            this.x = hardcoreMode;
            this.y = -1.0F;
            AdvancementTreeNode advancementtreenode = null;

            for (Advancement advancement : seedIn.getChildren())
            {
