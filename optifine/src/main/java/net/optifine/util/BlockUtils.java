...
public class BlockUtils
{
    ...
    public static boolean shouldSideBeRendered(BlockState blockStateIn, IBlockReader blockReaderIn, BlockPos blockPosIn, Direction facingIn, RenderEnv renderEnv)
    {
        ...
        if (blockStateIn.isSideInvisible(blockstate, facingIn))
        {
            return false;
        }
        else
    ...
    public static BlockState getBlockState(Block block, int metadata)
    {
        ...
        if (metadata >= 0 && metadata < list.size())
        {
            BlockState blockstate = list.get(metadata);
            return blockstate;
        }
        else
        {
    ...
    {
        return Block.isOpaque(stateIn.getCollisionShape(blockReaderIn, posIn));
    }

    public static Collection<IProperty<?>> getProperties(BlockState blockState)
    {
