...
public class Class3592 implements Comparator<Entity> {
   ...
   public int compare(Entity var1, Entity var2) {
      float var5 = RotationHelper.getAngleDistance(RotationHelper.getRotationsToEntity(var1).yaw, BowAimbot.mc.player.rotationYaw);
      float var6 = RotationHelper.getAngleDistance(RotationHelper.getRotationsToEntity(var2).yaw, BowAimbot.mc.player.rotationYaw);
      if (!(var5 - var6 < 0.0F)) {
         if (var5 - var6 != 0.0F) {
            return 1;
         } else {
