...
public class RearView extends Module {
    ...
    public boolean isEntityWithinViewAngle(LivingEntity targetEntity) {
        float rotations = RotationHelper.getRotationsToEntityFrom(targetEntity, mc.player.getPosX(),
        ...
        if (framebuffer != null) {
            if (this.isEnabled()) {
                if (!Minecraft.getInstance().gameSettings.hideGUI) {
                    ...
                    GL11.glScaled(1.0 / mc.mainWindow.getGuiScaleFactor() * (double) GuiManager.scaleFactor,
                            1.0 / mc.mainWindow.getGuiScaleFactor() * (double) GuiManager.scaleFactor, 1.0);
