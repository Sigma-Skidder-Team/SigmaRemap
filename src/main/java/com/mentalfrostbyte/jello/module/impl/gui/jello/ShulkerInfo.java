...
public class ShulkerInfo extends Module {
            ...
            for (Entity var5 : MultiUtilities.getEntitesInWorld()) {
                if (var5 instanceof ItemEntity) {
                    ...
                    if (!(var6.method4124().getItem() instanceof BlockItem)
                            || !(((BlockItem) var6.method4124().getItem()).getBlock() instanceof ShulkerBoxBlock)) {
                        return;
                    }
    ...
    public boolean method16672(ItemEntity var1) {
        if (mc.player.getDistance(var1) > 5.0F) {
            ...
            float var14 = RotationHelper.clampYawPitchChange(mc.player.rotationYaw,
                    (float) (Math.atan2(var10, var6) * 180.0 / Math.PI) - 90.0F, 360.0F);
            float var15 = RotationHelper.clampYawPitchChange(mc.player.rotationPitch,
