...
public class class_8838 extends DataFix {
   public class_8838(Schema var1, boolean var2) {
      super(var1, var2);
   }
      ...
      return TypeRewriteRule.seq(
         this.fixTypeEverywhereTyped(
            ...
            var3x -> var3x.updateTyped(
                  ...
                  var6,
                  var2x -> {
                     Optional var5x = var2x.getOptionalTyped(var9)
                        .<Dynamic>flatMap(var0xx -> var0xx.write().result())
                        .flatMap(var0xx -> var0xx.asStreamOpt().result());
                     Dynamic var6x = (Dynamic)var2x.get(DSL.remainderFinder());
                     ...
                     if (!var7x) {
                        ...
                        Optional var9x = var6x.get("Biomes").asByteBufferOpt().result();
                        if (var9x.isPresent()) {
                        ...
                        Dynamic var15 = var6x;
                        List var16 = IntStream.range(0, 16).<ShortArrayList>mapToObj(var0xx -> new ShortArrayList()).collect(Collectors.toList());
                        if (var5x.isPresent()) {
                           ((Stream)var5x.get()).forEach(var1xx -> {
                              int var4x = var1xx.get("x").asInt(0);
                              int var5xx = var1xx.get("y").asInt(0);
                              int var6xx = var1xx.get("z").asInt(0);
                              short var7xx = method_40670(var4x, var5xx, var6xx);
                              ((ShortList)var16.get(var5xx >> 4)).add(var7xx);
                           });
                           var6x = var6x.set(
                              "ToBeTicked", var6x.createList(var16.stream().<Dynamic>map(var1xx -> var15.createList(var1xx.stream().map(var15::createShort))))
                           );
                        }

                        var8x = (Dynamic)DataFixUtils.orElse(var2x.set(DSL.remainderFinder(), var6x).write().result(), var6x);
                     }

                     return (Typed)((Pair)var6.readTyped(var8x).result().orElseThrow(() -> new IllegalStateException("Could not read the new chunk")))
                        .getFirst();
                  }
               )
         ),
         this.writeAndRead(
            "Structure biome inject", this.getInputSchema().getType(class_4002.field_19431), this.getOutputSchema().getType(class_4002.field_19431)
         )
      );
   }

   private static short method_40670(int var0, int var1, int var2) {
