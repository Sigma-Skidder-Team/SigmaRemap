package remapped;

import java.util.List;
import java.util.Random;

public class class_6576 extends class_7733 {
   private static String[] field_33518;

   public class_6576(int var1, Random var2, class_9616 var3, Direction var4) {
      super(class_2746.field_13443, var1);
      this.method_32519(var4);
      this.field_43750 = this.method_39333(var2);
      this.field_36435 = var3;
   }

   public class_6576(class_5799 var1, CompoundNBT var2) {
      super(class_2746.field_13443, var2);
   }

   @Override
   public void method_32527(class_7060 var1, List<class_7060> var2, Random var3) {
      Direction var6 = this.method_32518();
      if (var6 != Direction.field_818 && var6 != Direction.field_804) {
         this.method_39331((class_4960)var1, var2, var3, 1, 1);
      } else {
         this.method_39335((class_4960)var1, var2, var3, 1, 1);
      }
   }

   public static class_6576 method_30026(List<class_7060> var0, Random var1, int var2, int var3, int var4, Direction var5, int var6) {
      class_9616 var9 = class_9616.method_44386(var2, var3, var4, -1, -1, 0, 5, 5, 5, var5);
      return method_39332(var9) && class_7060.method_32502(var0, var9) == null ? new class_6576(var6, var1, var9, var5) : null;
   }

   @Override
   public boolean method_32501(class_700 var1, class_2033 var2, class_6541 var3, Random var4, class_9616 var5, class_2034 var6, BlockPos var7) {
      this.method_32511(var1, var5, 0, 0, 0, 4, 4, 4, true, var4, class_7965.method_36051());
      this.method_39330(var1, var4, var5, this.field_43750, 1, 1, 0);
      Direction var10 = this.method_32518();
      if (var10 != Direction.field_818 && var10 != Direction.field_804) {
         this.method_32500(var1, var5, 0, 1, 1, 0, 3, 3, field_36439, field_36439, false);
      } else {
         this.method_32500(var1, var5, 4, 1, 1, 4, 3, 3, field_36439, field_36439, false);
      }

      return true;
   }
}
