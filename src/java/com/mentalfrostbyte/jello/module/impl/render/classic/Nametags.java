        ...
        if (this.isEnabled()) {
            ...
            for (Entry<PlayerEntity, Vector2f> entry : this.playerScreenPositions.entrySet()) {
                ...
                if (!player.isInvisible()) {
                    ...
                    RenderUtil.renderBackgroundBox((float) x, (float) y, (float) width, 20.0F, MultiUtilities.applyAlpha(ClientColors.DEEP_TEAL.color, 0.5F));
                    RenderUtil.drawString(ResourceList.bold16, (float) (x + 3), (float) y, name, MultiUtilities.applyAlpha(ClientColors.DEEP_TEAL.color, 0.5F));
                    RenderUtil.drawString(ResourceList.bold16, (float) (x + 3), (float) (y - 1), name, ClientColors.LIGHT_GREYISH_BLUE.color);
                    GL11.glPopMatrix();
