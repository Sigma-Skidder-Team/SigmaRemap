...
public class AutoMiner extends Module {
   ...
   public void method16079(TickEvent var1) {
      if (this.isEnabled()) {
         if (mc.player.ticksExisted < 20) {
            ...
            label90: for (ChunkPos var9 : var5) {
            ...
            if (!var14 && mc.player.onGround && !Client.getInstance().getOrientation().method31742()
               ...
               Client.getInstance().notificationManager
                     .send(new Notification("AutoMiner", "Computing...", ResourceList.directionIconPNG));
               this.field23454 = new Thread(
                     () -> {
                              ...
                              if (var13x.size() > 1) {
                                 ...
                                 Client.getInstance().notificationManager
                                       .send(
                                             new Notification(
                                                   "AutoMiner",
                                                   "Solved in " + var13x.size() + " steps! ("
                                                         + this.field23453.field45900 + ":" + var11x + ")",
                        ...
                        try {
                           if (!var4x) {
                              Client.getInstance().notificationManager
                                    .send(new Notification("AutoMiner",
                                          "Could not fin any safe path. (" + var15.size() + ")",
                                          ResourceList.directionIconPNG));
                              Thread.sleep(1000L);
                           }
            ...
            if (this.isEnabled() && this.field23453 != null && this.field23454 != null) {
               ...
               Client.getInstance().notificationManager
                     .send(new Notification("AutoMiner", "Computing... (" + this.field23453.field45900 + ")",
                           ResourceList.directionIconPNG));
            }
         }
      }
   ...
   private void method16082() {
      ...
      for (int var4 = 0; var4 <= 315; var4 += 45) {
         ...
         int var5 = var4 / 45;
         this.method16083((float) var3[var5].getRed() / 255.0F, (float) var3[var5].getGreen() / 255.0F,
               (float) var3[var5].getBlue() / 255.0F);
         GL11.glPopMatrix();
      }
      ...
      for (int var7 = 0; var7 <= 315; var7 += 45) {
         ...
         this.method16083((float) var6.getRed() / 255.0F, (float) var6.getGreen() / 255.0F,
               (float) var6.getBlue() / 255.0F);
         GL11.glPopMatrix();
      }
   }
