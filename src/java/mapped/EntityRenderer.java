...
public abstract class EntityRenderer<T extends Entity> {
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (Reflector.RenderNameplateEvent_Constructor.exists()) {
            ...
            if (object1 != ReflectorForge.EVENT_RESULT_DENY && (object1 == ReflectorForge.EVENT_RESULT_ALLOW || this.canRenderName((T) entityIn))) {
                ITextComponent itextcomponent = (ITextComponent) Reflector.call(object, Reflector.RenderNameplateEvent_getContent);
                this.renderName((T) entityIn, itextcomponent, matrixStackIn, bufferIn, packedLightIn);
            }
        } else if (this.canRenderName((T) entityIn)) {
            this.renderName((T) entityIn, entityIn.getDisplayName(), matrixStackIn, bufferIn, packedLightIn);
        }
    }
    ...
    public void renderName(T var1, ITextComponent var2, MatrixStack var3, IRenderTypeBuffer var4, int var5) {
