...
public class Class1870 extends Class1868
{
    ...
    public float method7120(final int n, final int n2) {
        ...
        final int n5 = n % 2;
        final int n6 = n2 % 2;
        float a = MathHelper.clamp(100.0f - MathHelper.method35640((float)(n * n + n2 * n2)) * 8.0f, -100.0f, 80.0f);
        for (int i = -12; i <= 12; ++i) {
            for (int j = -12; j <= 12; ++j) {
                final long n7 = n3 + i;
                final long n8 = n4 + j;
                if (n7 * n7 + n8 * n8 > 4096L) {
                    if (this.field10198.method30766((double)n7, (double)n8) < -0.8999999761581421) {
                        ...
                        a = Math.max(a, MathHelper.clamp(100.0f - MathHelper.method35640(n10 * n10 + n11 * n11) * n9, -100.0f, 80.0f));
                    }
                }
            }
        }
