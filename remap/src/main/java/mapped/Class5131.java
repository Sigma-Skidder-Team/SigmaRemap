...
public class Class5131 implements Class5113
{
    ...
    public static <T> Class5131 method16039(final Dynamic<T> dynamic) {
        return new Class5131(dynamic.get("state").map((Function) IFluidState::deserialize).orElse(Class7558.field29974.getDefaultState()), dynamic.get("requires_block_below").asBoolean(true), dynamic.get("rock_count").asInt(4), dynamic.get("hole_count").asInt(1), (Set<Block>)ImmutableSet.copyOf((Collection)dynamic.get("valid_blocks").asList(dynamic2 -> Registry.BLOCK.getOrDefault(new ResourceLocation(dynamic2.asString("minecraft:air"))))));
    }
}
