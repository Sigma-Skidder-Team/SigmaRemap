...
public class Class5144 implements Class5113
{
    ...
    public static <T> Class5144 method16063(final Dynamic<T> dynamic) {
        return new Class5144(Registry.field218.getOrDefault(new ResourceLocation(dynamic.get("state_provider").get("type").asString().orElseThrow(RuntimeException::new))).method30718((Dynamic<?>)dynamic.get("state_provider").orElseEmptyMap()), Registry.field219.getOrDefault(new ResourceLocation(dynamic.get("block_placer").get("type").asString().orElseThrow(RuntimeException::new))).method27915((Dynamic<?>)dynamic.get("block_placer").orElseEmptyMap()), (Set<Block>)dynamic.get("whitelist").asList((Function) BlockState::deserialize).stream().map(BlockState::getBlock).collect(Collectors.toSet()), Sets.newHashSet((Iterable)dynamic.get("blacklist").asList((Function) BlockState::deserialize)), dynamic.get("tries").asInt(128), dynamic.get("xspread").asInt(7), dynamic.get("yspread").asInt(3), dynamic.get("zspread").asInt(7), dynamic.get("can_replace").asBoolean(false), dynamic.get("project").asBoolean(true), dynamic.get("need_water").asBoolean(false));
    }
}
