...
public class Draft_6455 extends Draft {
    ...
    public IExtension getExtension() {
        return this.currentDecodingExtension;
    }
    ...
    private TranslatedPayloadMetaData translateSingleFramePayloadLength(ByteBuffer buffer,
            ...
            throws InvalidFrameException, IncompleteException, org.java_websocket.exceptions.LimitExceededException {
    ...
    private void logRuntimeException(final WebSocketImpl webSocketImpl, final RuntimeException ex) {
        log.error("Runtime exception during onWebsocketMessage ", ex);
        webSocketImpl.getWebSocketListener().onWebsocketError(webSocketImpl, ex);
    }
    ...
    private void addToBufferList(final ByteBuffer byteBuffer) {
        synchronized (this.byteBufferList) {
            this.byteBufferList.add(byteBuffer);
        }
    }

    private void checkBufferLimit() throws org.java_websocket.exceptions.LimitExceededException {
        if (this.getByteBufferListSize() <= this.maxFrameSize) {
            return;
        }
    ...
    public String toString() {
        ...
        if (this.getExtension() != null) {
            str = str + " extension: " + this.getExtension().toString();
        }
        if (this.getProtocol() != null) {
            str = str + " protocol: " + this.getProtocol().toString();
        }
        return str + " max frame size: " + this.maxFrameSize;
    }
    ...
    public boolean equals(final Object o) {
        ...
        final Draft_6455 draft6455 = (Draft_6455) o;
        if (this.maxFrameSize == draft6455.getMaxFrameSize()) {
            if (this.currentDecodingExtension == null) {
                if (draft6455.getExtension() != null) {
                    return false;
                }
            } else if (!this.currentDecodingExtension.equals(draft6455.getExtension())) {
                return false;
            }
            return (this.protocol == null) ? (draft6455.getProtocol() == null) : this.protocol.equals(draft6455.getProtocol());
        }
        return false;
    }
    ...
    @Override
    public int hashCode() {
        return 31 * (31 * ((this.currentDecodingExtension == null) ? 0 : this.currentDecodingExtension.hashCode()) + ((this.protocol == null) ? 0 : this.protocol.hashCode())) + (this.maxFrameSize ^ this.maxFrameSize >>> 32);
    }
    ...
    private long getByteBufferListSize() {
        long n = 0L;
        synchronized (this.byteBufferList) {
            final Iterator<ByteBuffer> iterator = this.byteBufferList.iterator();
